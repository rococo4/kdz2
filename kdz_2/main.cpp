...
#include <chrono>
#include <fstream>
std::vector<std::vector<long long>> get_times(std::string& s, bool insert = false);
...
std::vector<std::string> get_all_possible_strings(const std::string& find, std::string& string) {
    std::vector<std::string> all_possible_strings;
...
void kmp_search_refined_faces(std::string find_with_questions, std::string string) {
...
void insert_symbols(std::string& s) {
    srand(time(nullptr));
    int count = rand() % 4;
...
std::vector<std::vector<long long int>> get_times(std::string& s, bool insert) {
    int step = 100;
    srand(time(nullptr));
    int pos = rand() % 3000;
    ...
    for (auto j = 0; j < 10; j++) {
        while (step <= 3000) {
            ...
            if (insert) {
                insert_symbols(sub_str);
            }

            start = std::chrono::high_resolution_clock::now();
            naive_search(sub_str, s);
            elapsed = std::chrono::high_resolution_clock::now() - start;
            times[i][0] += std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed).count();

            start = std::chrono::high_resolution_clock::now();
            kmp_search(sub_str, s);
            elapsed = std::chrono::high_resolution_clock::now() - start;
            times[i][1] += std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed).count();

            start = std::chrono::high_resolution_clock::now();
            kmp_search_refined_faces(sub_str, s);
            elapsed = std::chrono::high_resolution_clock::now() - start;
            times[i][2] += std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed).count();
...
std::vector<std::vector<long long>> count_times_for_100000_four_no_insertions() {
    std::string s = generate_four_letters(100000);
    std::vector<std::vector<long long int>> times = get_times(s);
    return times;
}
std::vector<std::vector<long long>> count_times_for_10000_bin_insertions() {

    std::string s = generate_binary(10000);
    std::vector<std::vector<long long int>> times = get_times(s, true);
    return times;
}
std::vector<std::vector<long long>> count_times_for_100000_bin_insertions() {
    std::string s = generate_binary(100000);
    std::vector<std::vector<long long int>> times = get_times(s, true);
    return times;
}
std::vector<std::vector<long long>> count_times_for_10000_four_insertions() {
    int step = 100;
    int pos = 100;
    std::string s = generate_four_letters(10000);
    std::vector<std::vector<long long int>> times = get_times(s, true);
...
std::vector<std::vector<long long>> count_times_for_100000_four_insertions() {
    int step = 100;
    int pos = 1000;
    std::string s = generate_four_letters(100000);
    std::vector<std::vector<long long int>> times = get_times(s, true);
...
void write_in_file(const std::string& name, const std::vector<std::vector<long long>>& time) {
    std::ofstream file;
    file.open(name);
    for (auto i : time) {
        for (auto j = 0; j < 3; j++) {
...
int main() {
    ...
    write_in_file("count_times_for_10000_bin_insertions.csv",
                  count_times_for_10000_bin_insertions());
    write_in_file("count_times_for_10000_four_insertions.csv",
                  count_times_for_10000_four_insertions());
    write_in_file("count_times_for_100000_bin_insertions.csv",
                  count_times_for_100000_bin_insertions());
    write_in_file("count_times_for_100000_four_insertions.csv",
                  count_times_for_100000_four_no_insertions());
}